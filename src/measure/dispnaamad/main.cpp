...
void dispersion_naamad(problem_param* p)
{
  ...
  dist_max = p0[0] - p->pts.domain_bound[0];
  ...
  dist_max     = std::max(dist_max, p->pts.domain_bound[2] - p0[0]);
  ...
  for (u64 i = p->pts.size(); i > 0;) {
    ...
    extent_low = p->pts.domain_bound[0];
    extent_up  = p->pts.domain_bound[3];
    ...
    dist         = p0[1] - p->pts.domain_bound[1];
    box.area     = (extent_up - extent_low) * dist;
    box_max.area = std::max(box_max.area, box.area);
    ++p->measures->boxcount;
  }
  ...
  for (u64 i = 0; i < p->pts.size(); ++i) {
    ...
    if (p1 == nullptr) {
      p1 = &p->pts.domain_bound[0];
    }

    // accumulate empty box area
    box.area = (p2[0] - p1[0]) * (p->pts.domain_bound[3] - p0[1]);
    assert(box.area >= 0.);
    box_max.area = std::max(box_max.area, box.area);
    ++p->measures->boxcount;
  }
