...
/* generated by ./script/configure-file.py using template src/measure/dispcombinatorial/manpage.hpp.in with data src/measure/dispcombinatorial/manpage.1.pre. Manual edits will be OVERWRITTEN UNCONDITIONALLY. */

namespace dptk
{
 std::string manpage = R"V0G0N(DISPCOMBINATORIAL(1)      Dispersion Toolkit Manuals      DISPCOMBINATORIAL(1)
...
DESCRIPTION
       Computes  dispersion, n * dispersion, the number of empty boxes, and/or
       a list of empty boxes of a given point set with cardinality n.
...
OPTIONS
       ...
       --boxes
              Feeds  all empty boxes of P_i, including interiour and exteriour
              boxes, to output.  With the smallest point  of  a  box  B  being
              (low_0 low_1), i.e. no other point of B has smaller coordinates,
              and with the upper point of B being (up_0 up_1),  i.e. no  other
              point  of  B  has greater coordinates, B is formatted as a tuple
              (low_0 low_1 up_0 up_1) on a distinct line of the output.

       --interior-boxes
              Similar to the option --boxes with the exception of B being  re‐
              stricted  to  integriour  boxes.  A Box is integriour if it does
              not intersect with the domain boundary of P_i.

       --greatest-box
              Similar to the option --boxes with the exception of B being  the
              first greatest empty box found during the exhaustive search.
